...
class CIF_Lister(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        e_features = e_features.view(-1, 41) if self.src in [
            'CGCNN', 'NEW'] else e_features.view(-1, 9)
        ...
        graph_crystal = Data(x=n_features, y=y, edge_attr=e_features, edge_index=a_matrix, global_feature=enc_compo, cluster=groups,
                             num_atoms=torch.tensor([len(n_features)]).float(), coords=coordinates, the_idx=torch.tensor([float(i)]))
...
class CIF_Dataset(Dataset):
    def __init__(self, part_data=None, norm_obj=None, normalization=None, max_num_nbr=12, radius=8, dmin=0, step=0.2, cls_num=3, root_dir='DATA/CIF-DATA/'):
        ...
        self.clusterizer = SPCL(
            n_clusters=cls_num, random_state=None, assign_labels='discretize')
    ...
    def __getitem__(self, idx):
        ...
        atom_fea = np.vstack([self.ari.get_atom_fea(
        ...
        for nbr in all_nbrs:
            if len(nbr) < self.max_num_nbr:
                ...
                nbr_fea.append(list(
                    map(lambda x: x[1], nbr)) + [self.radius + 1.] * (self.max_num_nbr - len(nbr)))
            else:
                nbr_fea_idx.append(
    ...
    def format_adj_matrix(self, adj_matrix):
        ...
        all_src_nodes = torch.tensor(
            [[x]*adj_matrix.shape[1] for x in src_list]).view(-1).long().unsqueeze(0)
